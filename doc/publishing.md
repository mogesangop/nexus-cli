...
- **npm**:`@mogesangop/nexus-cli`,一个 thin wrapper 包,`postinstall`
...
| npm | `@mogesangop/nexus-cli` | 跨平台 npm 生态用户(CI 镜像、devcontainer) | 一个 scoped 包,`postinstall` 拉二进制 |
...
1. 用户 `npm i -g @mogesangop/nexus-cli`,npm 把包解压到全局
...
1. 打开 https://www.npmjs.com/signup 注册账号。
   - 用户名建议用 `mogesangop`(与 GitHub 用户名一致)。
   - 注册后该账号自动拥有 `@mogesangop` scope,可以发布
     `@mogesangop/<任意包名>`。
...
> **为什么用 scoped 包名 `@mogesangop/nexus-cli`?**
...
> "Read and write" + 包名 `@mogesangop/nexus-cli` + 过期时间。比
...
# 4. 验证
npm view @mogesangop/nexus-cli version
# 期望输出: 1.1.0
```
...
| Email address | `release@mogesangop.local` | 任意,不用于通信 |
...
Name-Email: release@mogesangop.local
Expire-Date: 0
%commit
EOF
```
...
uid         [ultimate] nexus-cli release <release@mogesangop.local>
```
...
- 用户 `npm i @mogesangop/nexus-cli@latest` 不会装到 RC
  (npm 的 `latest` dist-tag 不指向预发布版本),需显式
  `npm i @mogesangop/nexus-cli@1.2.0-rc.1`。
...
# 2. 在干净环境装一下
npx --package=@mogesangop/nexus-cli@1.1.0 -- nexus-cli --help
# 或
npm i -g @mogesangop/nexus-cli@1.1.0
nexus-cli --help
...
| `npm` job: `npm ERR! You must be logged in to publish packages` | scope 归属错误(账号不属于 `@mogesangop` org) | 确认 npm 账号是 `mogesangop`,或改用个人 scope |
| `npm install` 后 `nexus-cli: binary not installed` | postinstall 被跳过(如 `--ignore-scripts` 安装)或下载失败 | 重新 `npm rebuild @mogesangop/nexus-cli`;检查网络能否访问 github.com |
...
3. 检查 npm 包历史是否有异常发布(`npm view @mogesangop/nexus-cli
   time`)。
4. 如有恶意版本,`npm deprecate @mogesangop/nexus-cli@<ver>
...
```sh
# npm
npm view @mogesangop/nexus-cli versions --json
...
```sh
# 72 小时内可彻底取消发布某版本
npm unpublish @mogesangop/nexus-cli@1.1.0

# 超过 72 小时只能标记弃用(用户安装时会看到警告)
npm deprecate @mogesangop/nexus-cli@1.1.0 "security issue, upgrade to 1.1.1"

# 如果整个包都要弃用(慎用)
npm deprecate @mogesangop/nexus-cli "package deprecated, see README"
```
