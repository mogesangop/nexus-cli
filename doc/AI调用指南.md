...
nexus-cli user create-readonly --config config.yaml \
  ...
  --dry-run \
  --output json
nexus-cli repo lifecycle preview --config config.yaml --repo protected-repo-example
```
...
nexus-cli user create-readonly --config config.yaml \
  ...
  --email alice@example.com \
  --yes
nexus-cli repo lifecycle run --config config.yaml --repo protected-repo-example --yes
```
